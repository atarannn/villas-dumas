...
function menuOpenAnim() {
  ...
  const links = document.querySelectorAll('[data-animation-links]');
...
function formOpenAnim(evt, reverseArg) {
  ...
  tl.fromTo(formInner, {autoAlpha: 0},
    {autoAlpha: 1,  ease: 'power4.easeInOut', duration: 0.8, clearProps: 'all' }, '<');
  tl.play();
}
    ...
    new FormMonster({
      elements: {
        ...
        fields: {
          ...
          tell: {
            inputWrapper: new SexyInput({ animation: 'none', $field: $form.querySelector('[data-field-tell]') }),
            rule: yup.string()
              .required(i18next.t('required'))
              .matches(/^[\d+]+$/, i18next.t('only_number')) // Додаємо перевірку на дозволені символи
              .trim(),
            defaultMessage: i18next.t('tell'),
            valid: false,
            error: [],
          }
        },
